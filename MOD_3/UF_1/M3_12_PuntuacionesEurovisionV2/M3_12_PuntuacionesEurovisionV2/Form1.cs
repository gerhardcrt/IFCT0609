...
namespace M3_12_PuntuacionesEurovisionV2
{
    public partial class frmPrincipal : Form
    {
        ...
        private void ClicarEnPuntuacion(object sender, EventArgs e)
        {
            ...
            if (cbSeleccionarParticipante.SelectedIndex != -1)
            {
                ...
                foreach (object obj in lbParticipantes.Items)
                {
                    ...
                    if (grupoActual.Nombre == cbSeleccionado && !grupoActual.puntuacionDemoscopia)
                    {
                        grupoActual.puntuacion += int.Parse(((Button)sender).Text);
                        grupoActual.puntuacionDemoscopia = true;
                        ((Button)sender).Enabled = false;
                    }
                }
                OrdenarListBox(lbParticipantes);     
                //cbSeleccionarParticipante.Items.RemoveAt(cbSeleccionarParticipante.SelectedIndex);
        ...
        private void ClicarEnPuntuacionPublico(object sender, EventArgs e)
        {
            Grupo grupoActual;
            string cbSeleccionado;
            ...
            if (cbSeleccionarParticipante.SelectedIndex != -1)
            {
                ...
                foreach (object obj in lbParticipantes.Items)
                {
                    ...
                    if (grupoActual.Nombre == cbSeleccionado && !grupoActual.puntuacionPublico)
                    {
                        grupoActual.puntuacion += int.Parse(((Button)sender).Text);
                        grupoActual.puntuacionPublico = true;
                        ((Button)sender).Enabled = false;
                    }
                }
                OrdenarListBox(lbParticipantes);
