...
namespace EditorTextoFontStyle
{
    public partial class Form1 : Form
    {
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            rtbContenido.Font = new Font(f.FontFamily, _tamanhoActualFuente, FontStyle.Regular);


            _estiloActual = 0;
            rtbContenido.Text = "Esto es un texto de prueba";
            rtbContenido.Select(0, 5);
        ...
        private void btnAumentarFuente_Click(object sender, EventArgs e)
        {
            ...
            //Solo permito aumentar la fuente si es menor de 40
            if (_tamanhoActualFuente < 40)
            {
        ...
        private void rtbContenido_SelectionChanged(object sender, EventArgs e)
        {
            ...
            if (!_protegerSeleccion)
            {
                ...
                {
                    btnNegrita.BackColor = Color.AliceBlue;
                    _estiloActual += 1;
                }
                else
                {
                ...
                {
                    btnCursiva.BackColor = Color.AliceBlue;
                    _estiloActual += 2;
                }
                else
                {
                ...
                {
                    btnSubrayado.BackColor = Color.AliceBlue;
                    _estiloActual += 4;
                }
                else
                {
        ...
        private void CambiarEstilo(object sender, EventArgs e)
        {
            ...
            if (botonPulsado == btnNegrita)
            {
                ...
                {
                    _estiloActual -= 1;
                    rtbContenido.SelectionFont = new Font(f.FontFamily, f.Size, (FontStyle)_estiloActual);
                ...
                else
                {
                    _estiloActual += 1;
                    rtbContenido.SelectionFont = new Font(f.FontFamily, f.Size, (FontStyle)_estiloActual);
                    botonPulsado.BackColor = Color.AliceBlue;
                }
            }
            ...
            if (rtbContenido.SelectedText.Length == 0)
            {
                ...
                if (posicionActual + 1 <= rtbContenido.Text.Length && char.IsLetter(rtbContenido.Text[posicionActual]) && posicionActual > 0 && char.IsLetter(rtbContenido.Text[posicionActual - 1]))
                {
                    ...
                    _protegerSeleccion = true;
                    rtbContenido.Select(inicioPalabra, longitudPalabra); //Selecciono la palabra
                    rtbContenido.SelectionFont = new Font(f.FontFamily, _tamanhoActualFuente, (FontStyle)_estiloActual); //aumento la fuente
                    rtbContenido.Select(posicionActual, 0); //Dejo el cursor donde estaba
                    _protegerSeleccion = false; //desactivo la protección del select
                    rtbContenido.Focus(); //devuelvo el foco al rtb porque estará en el botón al haberlo pulsado
                }
            ...
            {
                //Si hay algo selecionado: Cambio la fuente y recuepero el foco que está en el botón
                rtbContenido.SelectionFont = new Font(f.FontFamily, _tamanhoActualFuente, (FontStyle)_estiloActual);
                rtbContenido.Focus();
            }
        }
    }
